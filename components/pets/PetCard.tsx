...
export function PetCard({
  ...
  const colorPattern = [pet.species, pet.primaryColor, pet.furPattern]
    .filter(Boolean)
    .join(" / ");
      ...
      <View style={styles.body}>
        ...
        {pet.status === "available" ? (
          <View style={styles.availableBadge}>
            <Text style={styles.availableBadgeText}>Tersedia</Text>
          </View>
        ) : null}
        ...
        {typeof pet.visualScore === "number" ? (
          <Text style={styles.score}>
            Kecocokan visual {Math.round(pet.visualScore)}%
          </Text>
        ) : null}
        ...
        {showDetailButton ? (
          ...
          >
            <Text style={styles.detailButtonText}>Detail informasi</Text>
          </Pressable>
        ) : null}
      </View>
    </Pressable>
...
const styles = StyleSheet.create({
  ...
  availableBadgeText: { color: "#166534", fontSize: 11, fontWeight: "800" },
  ...
  detailButtonPressed: { opacity: 0.7 },
  detailButtonText: { color: "#0f766e", fontSize: 13, fontWeight: "800" },
});
