...
async function getSignedImageUrl(imagePath: string) {
  ...
  if (!exists) {
    throw new HttpsError(
      "not-found",
      "Foto referensi tidak ditemukan. Pilih ulang foto lalu coba lagi.",
    );
  }
  ...
  if (!isAllowedSearchImageMetadata(metadata)) {
    throw new HttpsError(
      "invalid-argument",
      "Foto referensi harus berupa gambar dengan ukuran maksimal 8 MB.",
    );
  }
...
export const analyzePetImage = onCall(
  { region: "asia-southeast2", secrets: [openRouterApiKey] },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError(
        "unauthenticated",
        "Login diperlukan untuk pencarian visual.",
      );
    }

    let imagePath: string;
    try {
      imagePath = normalizeSearchImagePath(
    ...
    try {
      ...
      throw new HttpsError(
        "internal",
        "Analisis gambar gagal. Gunakan filter manual atau coba foto lain.",
      );
    }
  },
);
...
export const notifyOwnerOnReport = onDocumentUpdated(
  ...
  async (event) => {
    ...
    await sendPushToUser(
      after.ownerId,
      "Laporan diterima",
      "Pengadopsi telah mengirim laporan kondisi hewan.",
    );
  },
);
